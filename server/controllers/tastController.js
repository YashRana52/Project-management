  ...
  try {
    ...
    if (!project) {
      ...
      return res.status(403).json({
        message: "You don't have admin previleges for this projects",
      });
    }
    if (
      assigneeId &&
...
export const updateTask = async (req, res) => {
  try {
    ...
    // Only team lead can update task
    ...
    // If assigneeId is changing, check permission
    if (
      req.body.assigneeId &&
    ...
    const updatedTask = await prisma.task.update({
      where: { id: req.params.id },
      data: req.body, // FRONTEND WILL CONTROL WHAT CAN BE UPDATED
    });
...
export const deleteTask = async (req, res) => {
  try {
    ...
    // Check: sab tasks same project ke ho
    const projectId = tasks[0].projectId;
    const isSameProject = tasks.every((task) => task.projectId === projectId);
    ...
    // Only team lead can delete task
    if (project.team_lead !== userId) {
      return res.status(403).json({
        message: "You don't have admin privileges for this project",
      });
